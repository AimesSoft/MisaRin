...
struct RustLibMisaRinPlugin::Impl {
  ...
  void HandleGetTextureInfo(const flutter::EncodableMap& args, std::unique_ptr<flutter::MethodResult<flutter::EncodableValue>> result) {
    ...
    // Check if we can reuse the current state
    if (s->handle != 0 && s->tid >= 0 && s->w == w && s->h == h && s->layers == layers) {
      result->Success(MakeResp(s, false)); return;
    }

    // 1. Sync Create/Ensure Engine
    bool is_new = false;
    if (s->handle == 0) {
      s->handle = engine_create(static_cast<uint32_t>(w), static_cast<uint32_t>(h));
      is_new = true;
    } else if (s->w != w || s->h != h) {
    ...
    if (s->handle == 0) { result->Error("fail", "engine_init"); return; }

    // 2. Sync GPU Surface creation with POOL REUSE
    if (s->tid < 0 || is_new) {
      if (s->tid >= 0) RecycleTextureLocked(s);

      void* sh = engine_create_present_dxgi_surface(s->handle, w, h);
      if (!sh) { result->Error("fail", "dxgi_init"); return; }
    ...
    result->Success(MakeResp(s, is_new));
  }
