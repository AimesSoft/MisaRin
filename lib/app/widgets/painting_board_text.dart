...
class _MoveTextCaretIntent extends Intent {
  const _MoveTextCaretIntent(this.delta, {this.expandSelection = false});
  final int delta;
...
mixin _PaintingBoardTextMixin on _PaintingBoardBase {
  ...
  CanvasTextOrientation _textOrientation = CanvasTextOrientation.horizontal;
  ...
  Widget? buildTextEditingOverlay() {
    ...
    final Widget editor = Container(
      decoration: BoxDecoration(border: Border.all(color: overlayColor)),
    ...
    final Widget confirmButton = Padding(
      ...
      child: Container(
        decoration: BoxDecoration(
          ...
          border: Border.all(color: confirmBorderColor),
        ...
        child: isFinalizing
            ...
            : IconButton(
                icon: Icon(FluentIcons.check_mark, color: overlayColor),
  ...
  void _handleTextHover(Offset boardLocal) {
    ...
    if (_hoverTextLayerId == targetLayer.id && _hoverTextBounds == bounds) {
      return;
    }
    setState(() {
  ...
  void _scheduleLiveTextLayerUpdate(
    ...
    Future<void> previous = _pendingTextLayerUpdate ?? Future<void>.value();
    if (waitForHistoryEntry != null) {
      previous = Future.wait<void>(<Future<void>>[
        previous,
        waitForHistoryEntry.then((_) {}),
      ]);
    }
    final Future<void> next = previous.then(
      (_) => _controller.updateTextLayer(layerId, data),
    );
    _pendingTextLayerUpdate = next.catchError((
      Object error,
  ...
  void _beginNewTextSession(Offset origin) {
    ...
    _textSession = _TextEditingSession(origin: origin, isNewLayer: true);
  ...
  Future<void> _beginEditExistingTextLayer(CanvasLayerInfo layer) async {
    ...
    _pendingTextLayerUpdate = null;
    final Future<_CanvasHistoryEntry> pendingHistory = _createHistoryEntry(
      backendPixelsSynced: _backend.isReady,
    );
  ...
  Future<void> _commitTextEditingSession() async {
    ...
    try {
      ...
      await _finalizeTextEditingSessionCleanup(session, initialFrameGeneration);
    }
  }
  ...
  CanvasLayerInfo? _hitTestTextLayer(Offset boardLocal) {
    final List<CanvasLayerInfo> layers = _controller.layers.toList(
      growable: false,
    );
  ...
  void _updateActiveTextSessionColor() {
    ...
    final CanvasTextData updated = session.data!.copyWith(color: _primaryColor);
...
class _TextEditorOverlayState extends State<_TextEditorOverlay> {
  ...
  Widget build(BuildContext context) {
    ...
    final double scaledFontSize = (widget.data.fontSize * widget.scale).clamp(
      0.1,
      4096.0,
    );
    final Map<LogicalKeySet, Intent> shortcuts = <LogicalKeySet, Intent>{
      ...
      LogicalKeySet(LogicalKeyboardKey.control, LogicalKeyboardKey.keyA):
          const _SelectAllTextOverlayIntent(),
      LogicalKeySet(LogicalKeyboardKey.arrowLeft): const _MoveTextCaretIntent(
        -1,
      ),
      LogicalKeySet(LogicalKeyboardKey.arrowRight): const _MoveTextCaretIntent(
        1,
      ),
            ...
            child: SizedBox(
              ...
              child: Stack(
                ...
                children: [
                  ...
                  Positioned(
                    left:
                        widget.data.align == TextAlign.right ||
                            widget.data.align == TextAlign.end ||
                            widget.data.align == TextAlign.center
                        ? -scaledFontSize
                        : 0,
                    right:
                        widget.data.align == TextAlign.left ||
                    ...
                    child: EditableText(
                      ...
                      style: TextStyle(
                        ...
                        letterSpacing: widget.data.letterSpacing * widget.scale,
                      ),
  ...
  void _handleMoveCaret(_MoveTextCaretIntent intent) {
    ...
    final int normalizedExtent = _clampOffset(
    ...
    final int targetExtent = (normalizedExtent + intent.delta).clamp(
      0,
      textLength,
    );
  ...
  void _selectFromGlobalPosition({
    ...
    state.renderEditable.selectPositionAt(from: from, to: to, cause: cause);
  }
