...
impl WorkerPool {
    ...
    pub fn new_raw(
        ...
        for _ in 0..initial {
            match pool.spawn() {
                Ok(worker) => pool.state.push(worker),
                Err(err) => {
                    pool.enable_inline_fallback(err);
                    break;
                }
            }
        }
...
impl WorkerPool {
    ...
    pub fn execute(&self, closure: TransferClosure<JsValue>) -> Result<(), JsValue> {
        if self.inline_only.get() {
            closure.apply_inline();
            return Ok(());
        }
        ...
        let worker = match self.worker() {
            Ok(worker) => worker,
            Err(err) => {
                self.enable_inline_fallback(err);
                closure.apply_inline();
                return Ok(());
            }
        };
...
impl WorkerPool {
    fn enable_inline_fallback(&self, err: JsValue) {
        if !self.inline_only.replace(true) {
            crate::console_error!(
                "WorkerPool disabled, falling back to inline execution: {:?}",
                err
            );
        }
    }
