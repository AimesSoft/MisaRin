...
fn with_brush_mut<T>(
    f: impl FnOnce(&mut Option<GpuBrushEngine>) -> Result<T, String>,
) -> Result<T, String> {
    #[cfg(not(target_family = "wasm"))]
    {
    ...
    #[cfg(target_family = "wasm")]
    {
        return GPU_BRUSH.with(|cell| f(&mut *cell.borrow_mut()));
    }
}

fn with_brush<T>(
    f: impl FnOnce(&Option<GpuBrushEngine>) -> Result<T, String>,
) -> Result<T, String> {
    #[cfg(not(target_family = "wasm"))]
    {
    ...
    #[cfg(target_family = "wasm")]
    {
        return GPU_BRUSH.with(|cell| f(&*cell.borrow()));
    }
}
